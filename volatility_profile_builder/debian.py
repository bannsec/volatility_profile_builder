...
import logging
logger = logging.getLogger("volatility_profile_builder:Debian")
...
def prompt_for_kernel(tag, tmpdirname):
    ...
    base_image = 'debian:' + tag
    client.images.pull(base_image)

    out = client.containers.run(base_image, '/bin/bash -c "apt-get update && echo AAAABBBBCCCCDDDD && apt-cache search linux-image-[0-9] | grep -v dbg"', remove=True)
    out = out.decode()
...
def build_profile(tag):
    ...
    client = docker.from_env()
    img_tag = "volatility_profile_builder:debian-{}".format(tag)

    # Create Dockerfile
    with open(os.path.join(here,"docker","Dockerfile_debian"), "r") as f:
        dockerfile = f.read()
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        print("Building image for debian:" + tag + ": " + kernel_version + " ... ", end='', flush=True)
